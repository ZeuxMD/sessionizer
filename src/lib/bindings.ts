...
export async function getConfig(): Promise<FrontendConfig> {
    return invoke<FrontendConfig>("get_config");
}

export async function finishSetup(): Promise<void> {
    return invoke<void>("finish_setup");
}

export async function updateSettings(timeoutMinutes: number, warningMinutes: number, action: string, autostartEnabled: boolean): Promise<void> {
    return invoke<void>("update_settings", { timeoutMinutes, warningMinutes, action, autostartEnabled });
}
...
export async function setupPassword(password: string, timeoutMinutes: number): Promise<string> {
    return invoke<string>("setup_password", { password, timeoutMinutes });
}
...
export async function resetPasswordWithRecovery(key: string, newPassword: string): Promise<boolean> {
    return invoke<boolean>("reset_password_with_recovery", { key, newPassword });
}

export async function changePassword(current: string, newPassword: string): Promise<boolean> {
    return invoke<boolean>("change_password", { current, newPassword });
}

export async function executeShutdown(action: string): Promise<void> {
    return invoke<void>("execute_shutdown", { action });
}

export async function startTimer(): Promise<void> {
    return invoke<void>("start_timer");
}

export async function clearTimer(): Promise<void> {
    return invoke<void>("clear_timer");
}

export async function clearTimerForNextLogin(): Promise<void> {
    return invoke<void>("clear_timer_for_next_login");
}
...
export async function markWarningNotificationSent(): Promise<void> {
    return invoke<void>("mark_warning_notification_sent");
}
